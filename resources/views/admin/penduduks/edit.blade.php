            ...
            <form method="POST" action="{{ route('admin.penduduks.update', [$penduduk->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama"
                        id="nama" value="{{ old('nama', $penduduk->nama) }}" required>
                    @if ($errors->has('nama'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('nik') ? 'is-invalid' : '' }}" type="text" name="nik"
                        id="nik" value="{{ old('nik', $penduduk->nik) }}" required>
                    @if ($errors->has('nik'))
                        <div class="invalid-feedback">
                            {{ $errors->first('nik') }}
                        </div>
                    ...
                    @foreach (App\Models\Penduduk::JENIS_KELAMIN_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('jenis_kelamin') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="jenis_kelamin_{{ $key }}"
                                ...
                                {{ old('jenis_kelamin', $penduduk->jenis_kelamin) === (string) $key ? 'checked' : '' }}
                                required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('provinsi') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($provinsis as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('provinsi_id') ? old('provinsi_id') : $penduduk->provinsi->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('kabupaten') ? 'is-invalid' : '' }}"
                        name="kabupaten_id" id="kabupaten_id" required>
                        @foreach ($kabupatens as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('kabupaten_id') ? old('kabupaten_id') : $penduduk->kabupaten->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('admin.penduduks.storeCKEditorImages') }}',
                                            true);
