...
@section('content')
    @can('penduduk_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="rounded-pill btn btn-success" href="{{ route('admin.penduduks.create') }}">
                                ...
                                <td>
                                    @can('penduduk_show')
                                        <a class="rounded-pill btn btn-xs btn-primary"
                                            href="{{ route('admin.penduduks.show', $penduduk->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    ...
                                    @can('penduduk_edit')
                                        <a class="rounded-pill btn btn-xs btn-info"
                                            href="{{ route('admin.penduduks.edit', $penduduk->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('penduduk_delete')
                                        <form action="{{ route('admin.penduduks.destroy', $penduduk->id) }}" method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-Penduduk:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
