...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.penduduk.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.penduduks.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('kabupaten') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($kabupatens as $id => $entry)
                            <option value="{{ $id }}" {{ old('kabupaten_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
