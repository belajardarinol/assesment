...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.transaction-details.update', [$transactionDetail->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                        id="name" value="{{ old('name', $transactionDetail->name) }}" required>
                    @if ($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('value_idr') ? 'is-invalid' : '' }}" type="number"
                        name="value_idr" id="value_idr" value="{{ old('value_idr', $transactionDetail->value_idr) }}"
                        step="0.01" required>
                ...
                <div class="form-group">
                    ...
                    <div style="padding-bottom: 4px">
                        <span class="rounded-pill btn btn-info btn-xs select-all"
                            style="border-radius: 0">{{ trans('global.select_all') }}</span>
                        <span class="rounded-pill btn btn-info btn-xs deselect-all"
                    ...
                    <select class="form-control select2 {{ $errors->has('transactions') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($transactions as $id => $transaction)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('transactions', [])) || $transactionDetail->transactions->contains($id) ? 'selected' : '' }}>
                                {{ $transaction }}</option>
                        @endforeach
                    </select>
