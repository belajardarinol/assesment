...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.materi.title_singular') }}
        </div>
            ...
            <form method="POST" action="{{ route('admin.materis.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('sub_bab') ? 'is-invalid' : '' }}" name="sub_bab_id"
                        ...
                        @foreach ($sub_babs as $id => $entry)
                            <option value="{{ $id }}" {{ old('sub_bab_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('keterampilan_apoteker') ? 'is-invalid' : '' }}"
                        ...
                        value="{{ old('keterampilan_apoteker', '') }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('klasifikasis') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($klasifikasis as $id => $klasifikasi)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('klasifikasis', [])) ? 'selected' : '' }}>{{ $klasifikasi }}
                            </option>
                        @endforeach
                    </select>
