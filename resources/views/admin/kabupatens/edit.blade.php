...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.kabupatens.update', [$kabupaten->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('provinsi') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($provinsis as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('provinsi_id') ? old('provinsi_id') : $kabupaten->provinsi->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label class="required"
                        for="nama_kabupaten">{{ trans('cruds.kabupaten.fields.nama_kabupaten') }}</label>
                    <input class="form-control {{ $errors->has('nama_kabupaten') ? 'is-invalid' : '' }}" type="text"
                        name="nama_kabupaten" id="nama_kabupaten"
                        value="{{ old('nama_kabupaten', $kabupaten->nama_kabupaten) }}" required>
