...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.cpmk.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.cpmks.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('sub_cpmks') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($sub_cpmks as $id => $sub_cpmk)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('sub_cpmks', [])) ? 'selected' : '' }}>{{ $sub_cpmk }}</option>
                        @endforeach
                    </select>
