...
@section('content')
    @can('provinsi_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="rounded-pill btn btn-success" href="{{ route('admin.provinsis.create') }}">
                                ...
                                <td>
                                    @can('provinsi_show')
                                        <a class="rounded-pill btn btn-xs btn-primary"
                                            href="{{ route('admin.provinsis.show', $provinsi->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    ...
                                    @can('provinsi_edit')
                                        <a class="rounded-pill btn btn-xs btn-info"
                                            href="{{ route('admin.provinsis.edit', $provinsi->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('provinsi_delete')
                                        <form action="{{ route('admin.provinsis.destroy', $provinsi->id) }}" method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-Provinsi:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
