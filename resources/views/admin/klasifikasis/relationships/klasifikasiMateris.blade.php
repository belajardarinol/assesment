@extends('layouts.admin')
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.show') }} {{ trans('cruds.klasifikasi.title') }}
        </div>

        <div class="card-body">
            <div class="form-group">
                <div class="form-group">
                    <a class="btn btn-default" href="{{ route('admin.klasifikasis.index') }}">
                        {{ trans('global.back_to_list') }}
                    </a>
                </div>
                <table class="table table-bordered table-striped">
                    <tbody>
                        <tr>
                            <th>
                                {{ trans('cruds.klasifikasi.fields.id') }}
                            </th>
                            <td>
                                {{ $klasifikasi->id }}
                            </td>
                        </tr>
                        <tr>
                            <th>
                                {{ trans('cruds.klasifikasi.fields.kode_klasifikasi') }}
                            </th>
                            <td>
                                {{ $klasifikasi->kode_klasifikasi }}
                            </td>
                        </tr>
                        <tr>
                            <th>
                                {{ trans('cruds.klasifikasi.fields.klasifikasi') }}
                            </th>
                            <td>
                                {{ $klasifikasi->klasifikasi }}
                            </td>
                        </tr>
                        <tr>
                            <th>
                                {{ trans('cruds.klasifikasi.fields.kode_subkategori') }}
                            </th>
                            <td>
                                {{ $klasifikasi->kode_subkategori }}
                            </td>
                        </tr>
                        <tr>
                            <th>
                                {{ trans('cruds.klasifikasi.fields.subkategori') }}
                            </th>
                            <td>
                                {{ $klasifikasi->subkategori }}
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="form-group">
                    <a class="btn btn-default" href="{{ route('admin.klasifikasis.index') }}">
                        {{ trans('global.back_to_list') }}
                    </a>
                </div>
            </div>
        </div>
    </div>

    <div class="card">
        <div class="card-header">
            {{ trans('global.relatedData') }}
        </div>
        <ul class="nav nav-tabs" role="tablist" id="relationship-tabs">
            <li class="nav-item">
                <a class="nav-link" href="#klasifikasi_materis" role="tab" data-toggle="tab">
                    {{ trans('cruds.materi.title') }}
                </a>
            </li>
        </ul>
        <div class="tab-content">
            <div class="tab-pane" role="tabpanel" id="klasifikasi_materis">
                @includeIf('admin.klasifikasis.relationships.klasifikasiMateris', [
                    'materis' => $klasifikasi->klasifikasiMateris,
                ])
            </div>
        </div>
    </div>
@endsection
